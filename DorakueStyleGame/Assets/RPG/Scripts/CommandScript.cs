...
public class CommandScript : MonoBehaviour
{
    ...
    //メインプレイヤーのコマンドスクリプト
    private MainPlayerCommandScript mainPlayerCommandScript;
    ...
    void Awake()
    {
        ...
        mainPlayerCommandScript = GameObject.FindWithTag("Player").GetComponent<MainPlayerCommandScript>();
        //現在のコマンドを初期化
    ...
    private void Update()
    {
        ...
        if (Input.GetButtonDown("B_Button"))
        {
            ...
            {
                mainPlayerCommandScript.ExitCommand();
        ...
        {
            if (Input.anyKeyDown
                || !Mathf.Approximately(Input.GetAxis("LStickHorizontal"), 0.0f)
                || !Mathf.Approximately(Input.GetAxis("LStickVertical"), 0.0f)
                )
            {
                currentCommand = CommandMode.ItemPanel;
                useItemInformationPanel.SetActive(false);
